...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        // CGEventTap callbacks run on a background run loop. On recent macOS,
        // Text Input Source APIs used for unicode resolution assert the main queue
        // (`dispatch_assert_queue`). Hotkey use only needs physical key codes (EventType).
